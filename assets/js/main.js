  ...
  (function radar() {
    ...
    function draw(prog) {
      ...
      for (var ring = 1; ring <= 4; ring++) {
        ...
        for (var i = 0; i <= N; i++) { var p = pt(i % N, R * ring / 4); i ? ctx.lineTo(p[0], p[1]) : ctx.moveTo(p[0], p[1]); }
        ctx.stroke();
      }
      ...
      ctx.closePath();
      ctx.fillStyle = "rgba(25,212,255,0.30)"; ctx.fill();
      ctx.strokeStyle = "rgba(25,212,255,0.95)"; ctx.lineWidth = 2.2; ctx.stroke();
      for (var v = 0; v < N; v++) { var vp = pt(v, R * target[v] * prog); ctx.fillStyle = "#19d4ff"; ctx.beginPath(); ctx.arc(vp[0], vp[1], 3.4, 0, Math.PI * 2); ctx.fill(); }
    }
